...
import "./FilmBoTop.css"
import { Row, Col, Pagination } from "antd";
import { getFilmBoTopAll } from "../../Helper";
        ...
        if (data.data && filter) {
            ...
            if (filter.primary_release_year !== undefined || filter.with_genres !== undefined || filter.with_origin_country !== undefined) {
                if (filter.primary_release_year !== undefined) {
                    filteredMovies = filteredMovies.filter(movie => movie.first_air_date.startsWith(filter.primary_release_year))
                }
                ...
                <Row justify={"space-between"}>
                    <Col xxl={8} xl={8}>
                        <p className="tv-show__title">
                            Danh sách phim bộ được đề xuất xem
                        </p>
                    </Col>
                    <Col xxl={16} xl={16}>
                                    ...
                                    <Col xxl={6} xl={6} md={12} sm={12} xs={24} className="tv-show__item" key={`${item.genre_ids[0]}+${index}`}>
                                        <Link to={`/film_detail/${item.id}/${item.original_name}`}>
                                            <div className="tv-show-surro">
                                                ...
                                                <p className="tv-show__name">{item.name}</p>
                                            </div>
                                        </Link>
                                    </Col>
                                )
